...
- **Tableau/Power BI** (Optional for visualization if any charts were produced): For creating interactive dashboards that display the insights from the data analysis.
...
- A 15% readmission rate within 15 days pointed to areas for improvement in post-discharge care.
...
3. Run the SQL scripts to perform the analysis:
   - `Risk_Stratification.sql`
   - `Cohort_Analysis.sql`
   - `Workflow_Optimization.sql`
   - `Readmission_Analysis.sql`
   - `Appointment_Distribution.sql`
   
   *(Note: Include the file names or paths to each script as per your file organization.)*
